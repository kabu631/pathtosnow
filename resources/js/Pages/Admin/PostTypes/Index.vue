...
const del = useForm({})
function destroy(id) {
  if (confirm('Delete this post type?')) del.delete(`/admin/post-types/${id}`)
}

</script>

<template>
  <Head title="Post Types — Admin | PathToSnow Nepal" />
  <AdminLayout>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Post Types</h1>
        <p class="text-gray-500 text-sm mt-0.5">Manage blog & travel guide categories</p>
      </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="t in types" :key="t.id" class="hover:bg-gray-50 group">
            ...
            <td class="px-5 py-4">
              <code class="text-xs bg-gray-100 px-2 py-1 rounded font-mono block mb-1">/travel-guide/{{ t.slug }}</code>
              <code class="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded font-mono block">key: {{ t.type_key }}</code>
            </td>
            <td class="px-5 py-4">
              <div v-if="t.hero_image_url" class="h-10 w-16 bg-gray-100 rounded border border-gray-200 overflow-hidden">
                <img :src="t.hero_image_url" class="h-full w-full object-cover" />
              </div>
            ...
            <td class="px-5 py-4">
              <div class="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <Link :href="`/admin/post-types/${t.id}/edit`" class="text-blue-600 hover:text-blue-800 text-xs font-semibold">Edit</Link>
                <button @click="destroy(t.id)" class="text-red-500 hover:text-red-700 text-xs font-semibold">Delete</button>
              </div>
            </td>
          </tr>
          <tr v-if="!types.length">
            <td colspan="6" class="text-center py-12 text-gray-400">No post types found.</td>
          </tr>
        </tbody>
      </table>
    </div>
