<template>
<AdminLayout>
  <Head :title="`Manage Photos - ${location.name}`" />
  ...
  <div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2">
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <h2 class="text-lg font-bold text-slate-900 mb-4">Uploaded Photos ({{ location.images.length }})</h2>
        
        <div v-if="location.images.length" class="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div v-for="img in location.images" :key="img.id" class="group relative aspect-square rounded-lg overflow-hidden bg-slate-100 border border-slate-200">
        ...
        <div v-else class="text-center py-16 text-slate-400">
          ...
          <p>No photos uploaded to this location yet.</p>
        </div>
      </div>
    </div>
  </div>
...
function uploadPhotos() {
    uploadForm.post(`/admin/locations/${props.location.id}/images`, {
        preserveScroll: true,
...
function deleteImage(id) {
    if(confirm('Delete this photo?')) {
        router.delete(`/admin/locations/images/${id}`, {
            preserveScroll: true
        })
    }
}
