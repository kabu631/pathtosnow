...
<AdminLayout :title="booking.is_quotation ? `Quotation Request — ${booking.booking_reference}` : `Booking — ${booking.booking_reference}`">
    <template #actions>
    ...
    <div class="max-w-4xl grid md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-5">
            <!-- Quotation Banner -->
            <div v-if="booking.is_quotation" class="bg-indigo-50 border border-indigo-200 rounded-xl p-4 flex gap-3 items-center">
                <span class="text-2xl">📋</span>
                <div>
                    <h3 class="text-sm font-bold text-indigo-950">Quotation Request</h3>
                    <p class="text-xs text-indigo-700 mt-0.5">This customer has requested a custom quotation. Review their travel requirements below and follow up with a quote.</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                ...
                <dl class="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    <div><dt class="text-gray-400 text-xs">Package</dt><dd class="font-medium">{{ booking.package?.name ?? booking.custom_package_name }}</dd></div>
                    <div><dt class="text-gray-400 text-xs">Type</dt>
                        <dd><span :class="`type-${booking.package?.type || 'custom'}`" class="badge text-xs">{{ booking.package?.type ?? 'Custom Itinerary' }}</span></dd></div>
                    <div><dt class="text-gray-400 text-xs">Travel date</dt><dd class="font-medium">{{ fmt(booking.travel_date, 'date') }}</dd></div>
                    <div><dt class="text-gray-400 text-xs">Group size</dt><dd>{{ booking.group_size }} people</dd></div>
                    <div><dt class="text-gray-400 text-xs">Price/person</dt><dd>{{ booking.package_id ? `$${Number(booking.price_per_person).toFixed(2)}` : 'TBD' }}</dd></div>
                    <div><dt class="text-gray-400 text-xs">Total Est. Price</dt><dd class="font-bold text-lg text-[#E85D26]">{{ booking.package_id ? `$${Number(booking.total_price).toFixed(2)}` : 'TBD' }}</dd></div>
                </dl>
        ...
        <div class="space-y-4">
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h2 class="text-sm font-semibold text-gray-900 mb-3">{{ booking.is_quotation ? 'Quotation status' : 'Booking status' }}</h2>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                ...
                <p class="text-sm text-gray-700">{{ fmt(booking.created_at) }}</p>
                <p v-if="booking.confirmed_at" class="text-xs text-gray-400 mt-2">Confirmed</p>
                <p v-if="booking.confirmed_at" class="text-sm text-gray-700">{{ fmt(booking.confirmed_at) }}</p>
            </div>
            <div class="bg-white rounded-xl border border-red-100 p-5">
                ...
                <button @click="deleteBooking"
                    ...
                    🗑 {{ booking.is_quotation ? 'Delete Quotation Request' : 'Delete Booking' }}
                </button>
            </div>
        </div>
    </div>
...
// Format ISO dates into readable strings
function fmt(val, mode = 'datetime') {
    if (!val) return '—'
